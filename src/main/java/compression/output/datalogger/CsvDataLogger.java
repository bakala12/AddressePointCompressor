...
public class CsvDataLogger implements IDataLogger {
    ...
    @Override
    public void saveData(String problemName, int iterationNumber, double cost, int numberOfVehicles) {
        if(writer == null)
            throw new DataLoggerException("Logger is not open to write");
        String line = problemName +"," + iterationNumber+"," + cost +","+ numberOfVehicles;
        writer.println(line);
    }

...
Start-Process java -ArgumentList "-jar $pathToJar -i $currDir\$inputFile -o $currDir\$outputFile -d $currDir\$dataFile -p $currDir\$plotDirNormal -iter $Iterations -r $currDir\$resultFileNameNormal" -Wait -NoNewWindow
Start-Process java -ArgumentList "-jar $pathToJar -i $currDir\$inputFile -o $currDir\$outputFileCompression -d $currDir\$dataFileCompression -p $currDir\$plotDirCompression -iter $Iterations -c -r $currDir\$resultFileNameCompression" -Wait -NoNewWindow
